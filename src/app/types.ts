...
export type Employee = { id:number; name:string; dept:string; role:string; salary:number; expenses:number; status:"pending"|"calculated"|"paid"; paidDate?:string; commission?:number; netSalary?:number };
export type NormalRange = { param:string; unit:string; min:string; max:string; note:string };
export type LabTest = { id:number; code:string; name:string; nameEn:string; cat:string; priceOfficial:number; price:number; consumablesCost:number; priceCost:number; isL2L:boolean; kit:string; kitQty:number; kitUnit:string; kitThreshold:number; time:string; normalRanges?:NormalRange[] };
...
export type PurchaseRequest = { id:number; dept:string; requestedBy:string; date:string; items:PurchaseItem[]; totalAmount:number; status:"pending"|"approved"|"rejected"; approvedBy?:string; approvedDate?:string; rejectionReason?:string; note:string };
export type EmployeeAdvance = { id:number; empName:string; dept:string; amount:number; date:string; note:string; repaid:boolean; repaidDate?:string };
